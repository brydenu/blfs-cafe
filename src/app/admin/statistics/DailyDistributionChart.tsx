...
import { formatPacificDateLabel, getPacificDateString } from '@/lib/pacific-time';
...
export default function DailyDistributionChart({ data, timeframe }: DailyDistributionChartProps) {
  ...
  const chartData = data.map((item) => {
    ...
    return {
      date: isToday
        ? 'Today'
        : formatPacificDateLabel(item.date, { weekday: 'short', month: 'short', day: 'numeric' }),
      fullDate: formatPacificDateLabel(item.date, {
        weekday: 'long',
        month: 'long',
        day: 'numeric',
        year: 'numeric'
