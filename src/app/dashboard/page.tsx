...
import { LightningIcon, LightbulbIcon, ChartIcon } from "@/components/icons";
import OrderNowButton from "@/components/OrderNowButton";
import { getActiveQueueDrinkCount } from "@/lib/queue-count";
import { pageTitle } from "@/lib/metadata";
import { isStatisticsNewBadgeActive } from "@/lib/feature-badges";
    ...
    <main className="min-h-screen relative overflow-hidden flex flex-col items-center">
      ...
      <div className="relative z-10 w-full max-w-4xl p-6 space-y-6 pb-20">
        ...
        <div className="grid grid-cols-2 gap-4">
            <OrderNowButton initialQueueCount={initialQueueCount} />
            
            <Link href="/dashboard/quick-order" className="block h-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            <Link href="/dashboard/favorites" className="block">
                <button className="w-full bg-[#003355] hover:bg-[#002a4d] border border-white/10 text-white p-3 sm:p-4 rounded-2xl font-bold text-xs sm:text-sm shadow-md transition-all active:scale-[0.97] active:translate-y-[2px] flex items-center justify-center gap-1.5 sm:gap-2 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400 flex-shrink-0">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                    </svg>
                    <span className="truncate">
                      <span className="hidden sm:inline">Edit </span>Favorites
                    </span>
                </button>
            </Link>

            <Link href="/dashboard/statistics" className="block relative">
                {isStatisticsNewBadgeActive() && (
                    <span className="absolute -top-2 right-1 sm:right-2 z-10 bg-red-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-md border border-red-400/60 pointer-events-none">
                        New
                    </span>
                )}
                <button className="w-full bg-[#003355] hover:bg-[#002a4d] border border-white/10 text-white p-3 sm:p-4 rounded-2xl font-bold text-xs sm:text-sm shadow-md transition-all active:scale-[0.97] active:translate-y-[2px] flex items-center justify-center gap-1.5 sm:gap-2 cursor-pointer">
                    <ChartIcon size={18} className="text-[#32A5DC] flex-shrink-0" />
            ...
            <Link href="/dashboard/suggestions" className="block">
                <button className="w-full bg-[#003355] hover:bg-[#002a4d] border border-white/10 text-white p-3 sm:p-4 rounded-2xl font-bold text-xs sm:text-sm shadow-md transition-all active:scale-[0.97] active:translate-y-[2px] flex items-center justify-center gap-1.5 sm:gap-2 cursor-pointer">
                    <LightbulbIcon size={18} className="flex-shrink-0" />
            ...
            <Link href="/dashboard/settings" className="block">
                <button className="w-full bg-[#003355] hover:bg-[#002a4d] border border-white/10 text-white p-3 sm:p-4 rounded-2xl font-bold text-xs sm:text-sm shadow-md transition-all active:scale-[0.97] active:translate-y-[2px] flex items-center justify-center gap-1.5 sm:gap-2 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 flex-shrink-0">
